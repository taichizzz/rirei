...
Many students already have access to Claude, ChatGPT, Gemini, or another AI subscription through a personal or school plan, but cannot afford large API bills. When one coding agent reaches its usage limit, moving to another means explaining the task again and rebuilding its context. That is tedious, and missing details can leave the next agent confused about the work.

Rirei keeps one coding task alive across the officially installed provider CLIs you already use. It preserves project state, decisions, failed approaches, tests, and next steps in a compact provider-independent handoff, so you can switch to another available agent without starting over.

Rirei does not bypass provider limits, combine quotas, or convert subscriptions into API credentials. Each provider CLI remains responsible for its own authentication, billing, and usage policy. Concurrent multi-agent orchestration is also available as an advanced workflow: Rirei gives agents isolated Git worktrees and durable local messaging so they can work safely without overwriting one another.
...
The only additional model input is the prompt or handoff supplied at launch. Default handoffs are bounded to 1,200 characters and an estimated 300 input tokens using the common four-characters-per-token estimate, and the task request appears exactly once with no note-recording instruction. Full checkpoint patches remain local and are not inserted into handoffs. Every additional provider launch still starts a model turn, so unnecessary switching can cost more than continuing an existing provider session.

## Handoff benchmark

Rirei includes a five-task paired benchmark that compares a strong full-request baseline with the exact text generated by `relay handoff`. In the final hardened run, both conditions completed all five tasks correctly.

| Measure                        | Full-request baseline |  Rirei handoff |
| :----------------------------- | --------------------: | -------------: |
| Public and hidden tests passed |                   5/5 |            5/5 |
...
The Rirei condition used 15.5% more non-cached tokens overall and reduced wall time by 3.6%. Only one of five tasks reached the predeclared 20% improvement threshold, so the benchmark's decision rule did not pass. These fixed synthetic tasks are directional evidence, not proof that Rirei reduces cost or latency in general.

Run the harness tests and zero-provider dry run with `npm run benchmark`. See [`docs/benchmarks.md`](docs/benchmarks.md) for the protocol, archived results, limitations, and the separate 15-call execution command.

## Documentation

Detailed docs live in [`docs/`](docs/README.md):

- [Benchmarks](docs/benchmarks.md) — handoff protocol, archived results, limitations, and safe reproduction commands.
...
- [Relay Threads](docs/relay-threads.md) - local multi-agent messaging, limits, and delivery semantics.
